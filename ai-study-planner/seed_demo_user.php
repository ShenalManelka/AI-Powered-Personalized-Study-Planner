...
$user->studentProfile()->updateOrCreate(
    ['user_id' => $user->id],
    ['study_hours' => 25.5, 'attendance' => 88.0, 'sleep_hours' => 7.5, 'internet_usage' => 4.0]
);
...
// Assignments
$user->assignments()->firstOrCreate(['title' => 'BST Implementation', 'subject_id' => $sub1->id], ['deadline' => now()->addDays(2), 'priority' => 'high', 'status' => 'pending']);
$user->assignments()->firstOrCreate(['title' => 'Project Proposal', 'subject_id' => $sub2->id], ['deadline' => now()->addDays(5), 'priority' => 'medium', 'status' => 'completed']);
$user->assignments()->firstOrCreate(['title' => 'Neural Networks lab', 'subject_id' => $sub3->id], ['deadline' => now()->addDays(1), 'priority' => 'high', 'status' => 'pending']);

// Exams
$user->exams()->firstOrCreate(['title' => 'Midterm Exam', 'subject_id' => $sub3->id], ['exam_date' => now()->addDays(14), 'exam_type' => 'Midterm', 'status' => 'upcoming']);
$user->exams()->firstOrCreate(['title' => 'Final Presentation', 'subject_id' => $sub2->id], ['exam_date' => now()->subDays(30), 'exam_type' => 'Final', 'status' => 'completed', 'score' => 85.5]);
...
// Study Plan
$plan = $user->studyPlans()->firstOrCreate(['status' => 'active'], ['title' => 'Demo Plan', 'start_date' => now()->toDateString(), 'end_date' => now()->addDays(7)->toDateString()]);
...
echo "Demo data created for demo@test.com\n";
