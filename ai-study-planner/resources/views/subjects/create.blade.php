<x-app-layout>
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="mb-8">
            ...
            <p class="text-gray-600 mt-1">Create a new subject to track its assignments and exams.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <form action="{{ route('subjects.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    <x-input-label for="code" value="Subject Code (e.g. CS101)" class="text-sm font-semibold text-gray-700" />
                    <x-text-input id="code" name="code" required class="block w-full mt-2 border-gray-300 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                </div>
                
                <div>
                    <x-input-label for="name" value="Subject Name" class="text-sm font-semibold text-gray-700" />
                    <x-text-input id="name" name="name" required class="block w-full mt-2 border-gray-300 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                </div>
                
                <div>
                    <x-input-label for="description" value="Description (Optional)" class="text-sm font-semibold text-gray-700" />
                    <textarea id="description" name="description" rows="3" class="block w-full mt-2 rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"></textarea>
                </div>
